//
//  CDPhotoPickerBrowserPhotoScrollView.m
//  CDPhotoBrowser
//
...
@interface CDPhotoPickerBrowserPhotoScrollView () {
    CDPhotoPickerBrowserPhotoView *_tapView; // for background taps
    CGFloat maxScale;
    CGFloat minScale;
...
@implementation CDPhotoPickerBrowserPhotoScrollView

- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:CDPhotoImageDidStartLoad object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:CDPhotoImageDidFinishLoad object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:CDPhotoImageDidFailLoadWithError object:nil];
}
...
- (id)init {
    if ((self = [super init])) {
        ...
        _tapView = [[CDPhotoPickerBrowserPhotoView alloc] initWithFrame:self.bounds];
        ...
        // Image view
        _photoImageView = [[CDPhotoPickerBrowserPhotoImageView alloc] initWithFrame:CGRectZero];
        ...
        progressView.frame = CGRectMake(0, 0, CDPickerProgressViewW, CDPickerProgressViewH);
...
- (void)addObservers {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(imageDownloadStartAction)
                                                 name:CDPhotoImageDidStartLoad
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(imageDownloadDidFinishAction)
                                                 name:CDPhotoImageDidFinishLoad
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(imageDownloadDidFailAction)
                                                 name:CDPhotoImageDidFailLoadWithError
                                               object:nil];
}
...
- (void)setPhoto:(CDPhotoPickerBrowserPhoto *)photo {
    _photo = photo;
    ...
    if (photo.photoPath.length) {
        ...
        if (photoRange.location != NSNotFound){
            [[CDPhotoPickerDatas defaultPicker] getAssetsPhotoWithURLs:photo.photoURL callBack:^(UIImage *obj) {
            ...
            _photoImageView.frame = [CDPhotoRect setMaxMinZoomScalesForCurrentBoundWithImageView:_photoImageView];
