//
//  CDPhotoPickerBrowserViewController.m
//  CDPhotoBrowser
//
...
#import "CDPhotoPickerBrowserViewController.h"
#import "CDPhotoRect.h"
...
@interface CDPhotoPickerBrowserViewController () <UIScrollViewDelegate,CDPhotoPickerPhotoScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,CDPhotoPickerCustomToolBarViewDelegate>
...
@property (nonatomic, weak  ) CDImagePickerSelectView             *imagePickerSelectView;
@property (nonatomic, strong) CDPhotoPickerCustomToolBarView      *XGtoolbar;
@property (nonatomic, weak  ) CDPhotoPickerBrowserPhotoScrollView *cellScrollView;
...
@implementation CDPhotoPickerBrowserViewController
...
- (void)setShowType:(CDShowImageType)showType {
    _showType = showType;
    if (self.showType != CDShowImageTypeImagePicker){
...
- (void)setupCollectionView {
    if (!_collectionView) {
        ...
        flowLayout.minimumLineSpacing = CDPickerColletionViewPadding;
        flowLayout.minimumInteritemSpacing = 0;
        flowLayout.itemSize = self.view.frame.size;
        flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        
        CGRect frame = self.view.bounds;
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width + CDPickerColletionViewPadding,self.view.frame.size.height) collectionViewLayout:flowLayout];
...
- (UILabel *)pageLabel {
    if (!_pageLabel) {
        ...
        NSDictionary *metrics = @{@"ZLPickerPageCtrlH":@(CDPickerPageCtrlH)};
...
- (void)setupTopView {
    ...
    if ((!_imagePickerSelectView) && (!self.needHidenBar)) {
        CDImagePickerSelectView *imagePickerSelectView = [[CDImagePickerSelectView alloc] init];
...
- (void)selectedButtonTapped {
    CDPhotoAssets *selectAsset = ((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage]).asset;
    // 0. 九张图片数量限制
...
- (void)setupXGToolbar {
    if ((!_XGtoolbar)  && (!self.needHidenBar)) {
        ...
        _XGtoolbar = [[CDPhotoPickerCustomToolBarView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds) - height, CGRectGetWidth(self.view.bounds), height)];
        _XGtoolbar.delegate = self;
		_XGtoolbar.nightMode = self.nightMode;
        __weak CDPhotoPickerBrowserViewController * weakself = self;
        
        _XGtoolbar.getSizeBlock = ^(){
            CDPhotoPickerBrowserPhoto *photo = weakself.photos[weakself.currentPage];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.photos.count) {
        CDPhotoPickerBrowserPhoto *photo = nil;
        ...
        CDPhotoPickerBrowserPhotoScrollView *scrollView =  [[CDPhotoPickerBrowserPhotoScrollView alloc] init];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section {
    return CGSizeMake(CDPickerColletionViewPadding, self.view.frame.size.height);
}
...
- (void)updateSelectBtn {
    ...
    CDPhotoAssets *photoAsset = ((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage]).asset;
    
    for (CDPhotoAssets *asset in self.selectedAssets) {
...
#pragma mark - CDPhotoPickerCustomToolBarViewDelegate
...
- (void)prepareLeftAndRighImage {
    if (self.photos.count > 1) {
        if (self.currentPage == 0) {//从第一张进入
            if (!((CDPhotoPickerBrowserPhoto *)self.photos[1]).photoImage) {
                CDPhotoAssets *asset = ((CDPhotoPickerBrowserPhoto *)self.photos[1]).asset;
                ((CDPhotoPickerBrowserPhoto *)self.photos[1]).photoImage = [asset originImage];
            }
        } else if (self.currentPage == self.photos.count - 1) {//从第最后一张进入
            if (!((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage) {
                CDPhotoAssets *asset = ((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage - 1]).asset;
                ((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage = [asset originImage];
            }
        } else {
            if (!((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage + 1]).photoImage) {
                CDPhotoAssets *asset = ((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage + 1]).asset;
            ...
            if (!((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage) {
                CDPhotoAssets *asset1 = ((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage - 1]).asset;
                ((CDPhotoPickerBrowserPhoto *)self.photos[self.currentPage - 1]).photoImage = [asset1 originImage];
            }
        }
    }
...
- (id<CDPhotoPickerBrowserPhoto>)currentPhoto {
