//
//  CDPhotoPickerAssetsViewController.m
//  CDPhotoBrowser
//
...
@interface CDPhotoPickerAssetsViewController () <CDPhotoPickerCollectionViewDelegate,UICollectionViewDataSource,CDPhotoPickerBrowserViewControllerDataSource,CDPhotoPickerBrowserViewControllerDelegate>

// View
// 相片View
@property (nonatomic) CDPhotoPickerCollectionView *collectionView;
// 标记View
...
@implementation CDPhotoPickerAssetsViewController
...
- (void) setupPhotoBrowserInCasePreview:(BOOL)preview
    ...
    // 图片游览器
    CDPhotoPickerBrowserViewController *pickerBrowser = [[CDPhotoPickerBrowserViewController alloc] init];
...
- (void)setTopShowPhotoPicker:(BOOL)topShowPhotoPicker {
    ...
    if (self.topShowPhotoPicker == YES) {
        ...
        CDPhotoAssets *lgAsset = [[CDPhotoAssets alloc] init];
        [reSortArray insertObject:lgAsset atIndex:0];
        
        self.collectionView.status = CDPickerCollectionViewShowOrderStatusTimeAsc;
...
- (CDPhotoPickerCollectionView *)collectionView {
    if (!_collectionView) {
        ...
        CDPhotoPickerCollectionView *collectionView = [[CDPhotoPickerCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        // 时间置顶
        collectionView.status = CDPickerCollectionViewShowOrderStatusTimeDesc;
        collectionView.translatesAutoresizingMaskIntoConstraints = NO;
        [collectionView registerClass:[CDPhotoPickerCollectionViewCell class] forCellWithReuseIdentifier:_cellIdentifier];
        // 底部的View
        [collectionView registerClass:[CDPhotoPickerFooterCollectionReusableView class]  forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:_footerIdentifier];
...
- (void) setupAssets {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[CDPhotoPickerDatas defaultPicker] getGroupPhotosWithGroup:self.assetsGroup finished:^(NSArray *assets) {
...
- (void)setAssetsGroup:(CDPhotoPickerGroup *)assetsGroup {
...
#pragma mark - CDPhotoPickerCollectionViewDelegate
...
- (void) pickerCollectionViewDidSelected:(CDPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(CDPhotoAssets *)deleteAssets {
    ...
    if (self.selectPickerAssets.count == 0){
        ...
        for (CDPhotoAssets *selectAsset in arr) {
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.selectAssets.count > indexPath.item) {
        ...
        if ([self.selectAssets[indexPath.item] isKindOfClass:[CDPhotoAssets class]]) {
...
#pragma mark - CDPhotoPickerBrowserViewControllerDataSource
...
- (CDPhotoPickerBrowserPhoto *)photoBrowser:(CDPhotoPickerBrowserViewController *)pickerBrowser photoAtIndexPath:(NSIndexPath *)indexPath {
    
    CDPhotoAssets *imageObj = [[CDPhotoAssets alloc] init];
    ...
    // 包装下imageObj 成 CDPhotoPickerBrowserPhoto 传给数据源
    CDPhotoPickerBrowserPhoto *photo = [CDPhotoPickerBrowserPhoto photoAnyImageObjWith:imageObj];
    
    CDPhotoPickerCollectionViewCell *cell = (CDPhotoPickerCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    photo.thumbImage = cell.cellImage;
    
    return photo;
}

#pragma mark - CDPhotoPickerBrowserViewControllerDelegate
